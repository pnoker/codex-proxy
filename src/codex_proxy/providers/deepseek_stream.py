...
class DeepSeekStreamHandler:
    """Maps DeepSeek SSE stream to Codex Responses API events."""
    ...
    def process_stream(self, resp: requests.Response) -> None:
        ...
        try:
            ...
            logger.error(f"Error in DeepSeek stream processing: {e}")
        finally:
    ...
    def _handle_line(self, json_data: bytes) -> None:
        try:
            ...
            logger.debug(f"DeepSeek STREAM DELTA: {data}")
            ...
            logger.debug(f"Failed to parse DeepSeek stream line: {e}")
...
def stream_responses_loop(
    ...
    stream_handler = DeepSeekStreamHandler(
        handler, model, created_ts, request_metadata
    )
